...
$sql = "SELECT a.appointmentID, a.rating, a.comment, a.ratingDate, a.eventID, a.userID, u.userName, f.feedbackID, f.replyComment, f.feedbackStatus, f.replyDate
        FROM appointment a
...
.container { max-width:700px; margin:auto; background:#fff; padding:30px; border-radius:16px; box-shadow:0 15px 30px rgba(0,0,0,0.2);}
h2 { margin-top:0; }
...
<div class="container">
    ...
    <p><b>Donor:</b> <?php echo htmlspecialchars($feedback['userName']); ?></p>
    <p><b>Rating:</b> <?php echo $feedback['rating']; ?> / 5</p>
    <p><b>Comment:</b> <?php echo htmlspecialchars($feedback['comment']); ?></p>
    <p><b>Rating Date:</b> <?php echo $feedback['ratingDate']; ?></p>

    <form method="POST">
        <label for="replyComment">Reply Comment:</label>
        <textarea name="replyComment" id="replyComment" required><?php echo htmlspecialchars($feedback['replyComment'] ?? ''); ?></textarea>

        <label for="feedbackStatus">Feedback Status:</label>
        <select name="feedbackStatus" id="feedbackStatus" required>
            <option value="Replied" <?php echo ($feedback['feedbackStatus']=='Replied')?'selected':''; ?>>Replied</option>
